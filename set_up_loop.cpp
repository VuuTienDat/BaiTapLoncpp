 ...
 bool Loop_::InitData()
        {
         ...
         if(success == false){SDL_Log("Error Init Data.....\n");}
          return success;
        }

bool Loop_::loadBackground()
{
  bool ret = g_background.LoadImg("img\\Back_ground1.jpg",Renderer_);
  if(ret == false){return false;}
   return true;
}
vector <ThreatObjects*> Loop_::Make_threat_list()
    {
...
void Loop_:: set_up_Menu()
     {
        menu.set_up_Menu();
        menu.Set_Menu(Renderer_);
     }
  void Loop_:: close_object()
  {
      index.~graphic_();
      player.Freedom();
      exploit.Freedom();
